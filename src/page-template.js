const generateMemberCards = members => {
    ...
    const filterEng =  members.filter(member => member.getRole() === "Engineer")
    .map(intInfo => engineerCard(intInfo)); 

    // console.log(filterEng);

    const filterInt =  members.filter(member => member.getRole() === "Intern")
    .map(intInfo => internCard(intInfo)); 
    ...
    cardTemplate.push(filterEng); 
    cardTemplate.push(filterInt); 
